// UpsampData.cpp: implementation of the CUpsampData class.
//
//////////////////////////////////////////////////////////////////////
...
//*							\->CResample::resamp
//*				   CAMDemod::ProcessBlock
//*							\->CResample::resamp
//***************************************************************************

//***************************************************************************
//*			*** resamp ***     Fixed Point Version 
//***************************************************************************

TM_BOOL CResample::resamp(tNativeAcc freq_fxp, 
					   tNative jump, 
					   tNative* data_in, 
					   tNative length)
{
	TM_BOOL symbol_valid=FALSE;
	
	//m_interp.interp(data_in, freq_fxp, jump, symbol_valid, length);

		tNative bits=12;

	tNative i;
  
	tNative total_symbol_complex = 2 * length;
	...
	if(m_out_ptr<0) 
		m_out_ptr += total_symbol_complex;
...
/*				zr = -del0r + 3*del1r - 3*del2r + del3r;
				...
				if (m_out_ptr >= total_symbol_complex)
				{
					m_out_ptr=0;
	...
	if (symbol_valid)
		for (i=0; i< total_symbol_complex; i++)
